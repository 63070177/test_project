        ...
        <div class="row">
          <div class="col-9">
            <ul class="nav nav-underline">
              <li v-if="user.type == 'nurse'" class="nav-item" @click="goToRegis()"
                    ...
                    <div class="modal-content">
                      ...
                      <div class="modal-body">
                        <div class="mb-3 row" style="text-align: left;">
                          ...
                          <div class="col-sm-8">
                            <input type="text" class="form-control" id="inputText" v-model="firstName">
                          </div>
                        </div>
                        <div class="mb-3 row" style="text-align: left;">
                          <label for="inputFirstname" class="col-sm-3 col-form-label">นามสกุล</label>
                          <div class="col-sm-8">
                            <input type="text" class="form-control" id="inputText" v-model="lastName">
                          </div>
                        </div>
                        <div class="mb-3 row" style="text-align: left;">
                        ...
                        <div class="mb-3 row" style="text-align: left;">
                          ...
                          <div class="col-sm-8">
                            ...
                            <select class="form-select" v-model="cancerState">
                              <option value="" disabled>กรุณาเลือกระยะมะเร็ง</option>
                      ...
                      <div class="modal-footer">
                        ...
                        <button @click="newTreatment" type="button" class="btn btn-success"
                ...
                <tr v-for="p in displayedPosts" :key="p.HN">
                  ...
                  <td>
                    <button type="button" class="btn"
                      style="border-radius: 20px; background-color: #34495E; color: white;" @click="goToHis(p)">
                      See More
                      <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor"
...
export default {
  ...
  data() {
    return {
      ...
      infoCancer: ['มะเร็งปอด', 'มะเร็งกระเพาะอาหาร', 'มะเร็งลำไส้ใหญ่', 'มะเร็งตับ', 'มะเร็งตับอ่อน', 
      'มะเร็งต่อมไทรอยด์', 'มะเร็งไต', 'มะเร็งกระเพาะปัสสาวะ', 'มะเร็งอัณฑะ', 'มะเร็งต่อมลูกหมาก', 
      'มะเร็งถุงน้ำดี', 'มะเร็งมดลูก', 'มะเร็งเต้านม', 'มะเร็งรังไข่']
    }
  },
  mounted() {
  ...
  methods: {
    sortBloodInfo() {
      console.log(this.sortBlood)
      const data = {
        sortBlood: this.sortBlood
      }
      if (this.sortBlood != 'เรียงลำดับตาม HN') {
        axios.post(`http://localhost:3000/sortBloodresult`, data).then((response) => {
    ...
    newTreatment() {
      if (this.cancerType != '' && this.cancerState != '' && this.firstName != '' && this.lastName != '' && this.formula != '' && this.doctor != '') {
        const data = {
          formula: this.fomula,
        ...
        axios.post(`http://localhost:3000/createTreatment`, data).then((response) => {
          if (response.data == 'no'){
            alert('ไม่พบผู้ป่วยนี้ในระบบ')
          }else{
            this.$router.push(`/appointInfo/${this.$route.params.userId}/${response.data[0].HN}/${response.data[0].treatmentId}`)
          }   
        }).catch((error) => {
          console.log(error)
        })
    ...
    goToHis(p) {
      this.$router.push(`/appointInfo/${this.$route.params.userId}/${p.HN}/${p.treatmentId}`)
    },
    goToBlood() {
