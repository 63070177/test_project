            ...
            <ul class="nav nav-underline">
              ...
              <li class="nav-item" @click="goTonewHome()"
                ...
                <a class="nav-link active" aria-current="page" href="#"
                  style="color: #ffffff; font-size: large;">ผลเลือด</a>
              </li>
      ...
      <div class="bd-example-snippet bd-code-snippet" style="border: none;">
        ...
        <div class="card" style="margin: 20px;">
          <div class="card-body" style="text-align: left;">
            <button type="button" style="margin-bottom: 20px;" class="btn btn-outline-success" @click="goTonewHome()">
              <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor"
            ...
            <div class="container" style="margin-bottom: 20px;">
              <div class="row g-0">
                ...
                <div class="col-4">
                  <b>เพศ</b> {{ patient.gender }}
                </div>
                <div class="col-4">
                  <b>อายุ</b> {{ patient.age }} ปี
                </div>
              </div>
            </div>
            <div class="container" style="margin-bottom: 20px;">
            ...
            <div class="col-md-5 card" style="margin-bottom: 50px;">
              <div class="card-header" style="background-color: #90eeb7;"><b>ผลเลือด</b></div>
              <div class="card-body">
                <table class="table">
                  <thead>
                    <tr class="table">
                  ...
                  <tbody class="table">
                    <tr v-for="blood in bloodresult" :key="blood.brId">
                      <td>{{ blood.picture }}</td>
                      <td>
                        <a data-bs-toggle="modal" data-bs-target="#exampleModal4" @click="openIMG(blood)">
                          <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor"
                ...
                <div class="modal-content">
                  ...
                  <div class="modal-body">
                    <div class="card-body" style="text-align: left;">
                      ...
                      <div class="mb-3"
                        style="width: 80%; height: 450px; margin: auto; background-color: white; border: 1px solid #0A6B3A;">
                      ...
                      <form style="margin-top: 10px;">
                        <div class="row mb-3">
                          ...
                          <div class="col-sm-4" style="text-align: left;">
                            <div class="col" v-if="selectedBloodresult.status == 'รออนุมัติผลเลือด'">
                              <label v-if="selectedBloodresult.status == 'อนุมัติผลเลือด'">{{selectedBloodresult.status}}</label>
                              <select v-if="selectedBloodresult.status != 'อนุมัติผลเลือด'" class="form-select" v-model="status">
                                <option selected disabled>{{ selectedBloodresult.status }}</option>
                                <option value="yes" :disabled="selectedBloodresult.status == 'อนุมัติผลเลือด'">อนุมัติผลเลือด
                                </option>
                                <option value="no" :disabled="selectedBloodresult.status == 'ส่งผลเลือดอีกครั้ง'">ส่งผลเลือดอีกครั้ง
                                </option>
                              </select>
                            </div>
                            <div class="col" v-if="selectedBloodresult.status == 'อนุมัติผลเลือด' || selectedBloodresult.status == 'ส่งผลเลือดอีกครั้ง'">
                              <label>{{selectedBloodresult.status}}</label>
                            </div>
                          </div>
                        </div>
                        <div class="row mb-3" v-if="status == 'yes' || selectedBloodresult.status == 'อนุมัติผลเลือด'">
                          <label class="col-sm-2 col-form-label"><b>สูตรยา : </b></label>
                          <div class="col-sm-4" style="text-align: left;">{{ patient.formulaName }}
                            <div class="col">
                            </div>
                          </div>
                        </div>
                            ...
                            <div class="col">
                              <div class="form-check form-check-inline">
                                <input class="form-check-input" type="radio" name="inlineRadioOptions" id="inlineRadio1"
                                  v-model="medType" value="ฉีดเข้าเส้นเลือด">
                                <label class="form-check-label" for="inlineRadio1">ชนิดฉีด</label>
                              </div>
                              <div class="form-check form-check-inline">
                                <input class="form-check-input" type="radio" name="inlineRadioOptions" id="inlineRadio2"
                                  v-model="medType" value="รับประทาน">
                                <label class="form-check-label" for="inlineRadio2">ชนิดรับประทาน</label>
                              </div>
                            </div>
                          </div>
                        </div>
                        <div class="row mb-3" v-if="status == 'yes'|| selectedBloodresult.status == 'อนุมัติผลเลือด'">
                          <div class="col-sm-10" style="text-align: left;" v-for="(med, index) in medicine"
                            :key="med.medId">
                            <div class="row g-3 align-items-center">
                              <div class="col-sm-4">
                                <label for="inputPassword6" class="col-form-label">{{ med.medName }}</label>
                              </div>
                              <div class="col-sm-3">
                                <div class="input-group mb-3">
                                  <input type="text" class="form-control" placeholder="ปริมาณ"
                                    aria-label="Recipient's username" aria-describedby="basic-addon2"
                                    v-model="amount[index]">
                                  <span class="input-group-text" id="basic-addon2">มก.</span>
                                </div>
                              </div>
                              <div class="col-sm-2" v-if="medType == 'รับประทาน'">
                                <div class="input-group mb-3">
                                  <input type="text" class="form-control" placeholder="จำนวน"
                                    aria-label="Recipient's username" aria-describedby="basic-addon2"
                                    v-model="tab[index]">
                                  <span class="input-group-text" id="basic-addon2">เม็ด</span>
                                </div>
                              </div>
                            </div>
                          </div>
                      ...
                      <label class="col-sm-3 col-form-label"><b>ข้อเสนอแนะ</b></label><br/>
                      <label class="form-control" v-if="selectedBloodresult.status != 'รออนุมัติผลเลือด' && selectedBloodresult.suggestion == null"
                        >-</label>
                      <label class="form-control" v-else-if="selectedBloodresult.status != 'รออนุมัติผลเลือด'"
                        >{{selectedBloodresult.suggestion}}</label>
                      <textarea v-else-if="selectedBloodresult.status == 'รออนุมัติผลเลือด'" class="form-control" id="exampleFormControlTextarea1" rows="3"
                        v-model="suggestion"></textarea>
                    </div>
                  </div>
                  <div class="modal-footer" v-if="selectedBloodresult.status == 'รออนุมัติผลเลือด' && user.type == 'doctor'">
                    <button type="button" class="btn btn-danger" data-bs-dismiss="modal">ยกเลิก</button>
                    <button type="button" class="btn btn-success" data-bs-dismiss="modal"
                      @click="saveComment()">ตกลง</button>
                  </div>
                </div>
              </div>
            </div>
...
export default {
  ...
  mounted() {
    ...
    axios.get(`http://localhost:3000/patient/${HN}/${treatmentId}`).then((response) => {
    ...
    axios.get(`http://localhost:3000/myformula/${HN}/${treatmentId}`).then((response) => {
      this.patient["formulaName"] = response.data[0].formulaName
    }).catch((error) => {
      console.log(error)
    })
    axios.get(`http://localhost:3000/doctor/${HN}`).then((response) => {
      this.doctor = response.data[0]
    }).catch((error) => {
      console.log(error)
    })
    axios.get(`http://localhost:3000/bloodresult/patient/${HN}/${treatmentId}`).then((response) => {
      ...
      if (this.bloodresult[0].status == 'อนุมัติผลเลือด') {
        axios.get(`http://localhost:3000/treatment/giveMed/${treatmentId}`).then((response) => {
          console.log(response.data)
          for (let i = 0; i < response.data.length; i++) {
  ...
  methods: {
    ...
    openIMG(blood) {
      ...
      axios.get(`http://localhost:3000/medicine/${formulaId}`).then((response) => {
    ...
    saveComment() {
      ...
      axios.post(`http://localhost:3000/giveMed`, data2).then((response) => {
        console.log(response.data)
        alert('บันทึกสำเร็จ')
