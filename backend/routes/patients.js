...
router.post('/addPatient', async function (req, res, next) {
    ...
    try {
        ...
        const [rows3, f3] = await conn.query('select last_insert_id(HN) as HN from patient order by HN')
        let last_HN = rows3.pop()
        let HN = Number(last_HN.HN) + 1
        const [rows2, _] = await conn.query('insert into patient (HN, firstName, lastName, birthdate, gender, cancerType, cancerState, phoneNumber, doctorId. formulaId) values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', [padWithLeadingZeros(HN, 6), firstName, lastName, birthDate, gender, cancerType, Number(cancerState), phoneNumber, rows1[0].doctorId]);
        res.json(rows2)
        conn.commit()*/
        const [rows1, f1] = await conn.query('select doctorId from doctor where firstName = ? and lastName = ?', [doctor_firstName, doctor_lastName]);
        const [rows3, f3] = await conn.query('select last_insert_id(HN) as HN from patient order by HN')
        let last_HN = rows3.pop()
        let HN = Number(last_HN.HN) + 1
        const psw = birthDate.split('T')[0]
        const [row4, f4] = await conn.query('insert into user (userName, psw) value (?, ?)', [userName, psw])
        const [row2, f2] = await conn.query('insert into patient (userId, HN, firstName, lastName, birthDate, gender, phoneNumber, doctorId, formulaId) value (last_insert_id(), ?, ?, ?, ?, ?, ?, ?, ?)', [padWithLeadingZeros(HN, 6), firstName, lastName, birthDate, gender, phoneNumber, rows1[0].doctorId, formula])
        const [row5, f5] = await conn.query('insert into history (cancerType, cancerState, HN) value (?, ?, last_insert_id())', [cancerType, cancerState])
        //conn.commit()
    } catch (error) {
        conn.rollback()
        console.log(error)
...
router.post(`/sortInfo`, async function (req, res, next) {
    ...
    try {
        if (sortInfo == '1') {
            const [rows, f] = await pool.query('select * from patient join treatment on treatment.HN=patient.HN join bloodresult on treatment.treatmentId=bloodresult.treatmentId order by firstName ASC')
            data.push(rows)
        } else if (sortInfo == '2') {
            const [rows1, f1] = await pool.query('select * from patient join treatment on treatment.HN=patient.HN join bloodresult on treatment.treatmentId=bloodresult.treatmentId order by firstName desc')
            data.push(rows1)
        } else if (sortInfo == '3') {
            const [rows2, f2] = await pool.query('select * from patient join treatment on treatment.HN=patient.HN join bloodresult on treatment.treatmentId=bloodresult.treatmentId order by patient.HN ASC')
            data.push(rows2)
        } else if (sortInfo == '4') {
            const [rows3, f3] = await pool.query('select * from patient join treatment on treatment.HN=patient.HN join bloodresult on treatment.treatmentId=bloodresult.treatmentId order by patient.HN desc')
            data.push(rows3)
        }
        res.json(data)
    } catch (error) {
...
router.post(`/newPatient`, async function (req, res, next) {
    ...
    try {
        ...
        await conn.query(`insert into diseases (disease, usualMed, otherTreatment, HN) values (?, ?, ?, ?)`, [disease, usualMed, otherTreatment, HN])
        if (allergyHis == 'แพ้') {
            await conn.query(`insert into allergy (allergyDetail, HN) values (?, ?)`, [allergy, HN])
        }
        await conn.query(`insert into addictive (smoking, alcohol, nut, HN) values (?, ?, ?, ?)`, [smoking, alcohol, nut, HN])
        ...
        await conn.query(`insert into history (nationality, religious, marriageStatus, education, occupation, bloodGroup, IDcardAddress, currentAddress, contactAddress, contactPerson, relatedAs, fatherName, motherName, spouseName, HN) 
        values(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [nationality, religious, marriageStatus, education, occupation, bloodGroup, IDcardAddress, currentAddress, contactAddress, contactPerson, relatedAs, fatherName, motherName, spouseName, HN])
        conn.commit()
        res.send('success')
    } catch (error) {
        conn.rollback()
...
router.post(`/createTreatment`, async function (req, res, next) {
    ...
    const firstName = req.body.firstName
    const lastName = req.body.lastName
    const [row2, f2] = await pool.query(`select HN from patient where firstName = ? and lastName = ?`, [firstName, lastName])
    ...
    try {
        if (row2.length == 0) {
            ...
            await conn.query(`insert into treatment (cancerType, cancerState, HN, formulaId, doctorId) values (?, ?, ?, ?, ?)`, [cancerType, cancerState, row2[0].HN, row1[0].formulaId, row[0].doctorId])
...
router.post('/selectedPatient', async function (req, res, next) {
    ...
    const data = new Array();
    try {
        const [rows, f] = await pool.query('SELECT * FROM patient join treatment on treatment.HN=patient.HN join bloodresult on treatment.treatmentId=bloodresult.treatmentId WHERE patient.HN=?', selected)
        const [rows1, f1] = await pool.query('select * from patient join treatment on treatment.HN=patient.HN join bloodresult on treatment.treatmentId=bloodresult.treatmentId where patient.firstName like ? or patient.lastName like ?', [`%${selected}%`, `%${selected}%`])
        const [rows2, f2] = await pool.query('select * from patient join treatment on treatment.HN=patient.HN join bloodresult on treatment.treatmentId=bloodresult.treatmentId where treatment.cancerType like ?', [`%${selected}%`])
        if (selected == '') {
            const [row, _] = await pool.query('select * from patient join treatment on treatment.HN=patient.HN join bloodresult on treatment.treatmentId=bloodresult.treatmentId')
            data.push(row)
